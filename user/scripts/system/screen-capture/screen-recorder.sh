#!/usr/bin/env -S nix shell nixpkgs#bash --command bash

# Code is modified from https://github.com/raffaem/waybar-screenrecorder/


SIGNAL=1
pidf="/tmp/waybar-screenrecorder"
WF_RECORDER_OPTS="--audio"

#DISPLAY=$(slurp -r -o)
OUTPUTDIR="$XDG_VIDEOS_DIR/screen_recordings"

VIDEOEXT="mp4"

# Check if you're currently recording 
# Used to display the status in the bar
if [ "$1" == "status" ]; then
	if [ -s "$pidf" ]; then
		awk 'BEGIN{printf "{\"text\":\" \", \"tooltip\":\"Recording\\n"}
		NR==1{printf "PID: %s\\n", $0}
		NR==2{printf "Save to: %s\\n", $0}
		NR==3{printf "Log to: %s\"}\n", $0}' "$pidf"
	else
		#printf '{"text":" ", "tooltip":"Stopped"}\n'
		printf '{"text":" ", "tooltip":"Stopped"}\n'
	fi


# Command used to toggle recording on and off
elif [ "$1" == "toggle" ]; then
	# Check if if recording
	if [ -s "$pidf" ]; then
		pid=$(awk 'NR==1' "$pidf")
		kill "$pid"
		printf "" > "$pidf"
	else
		#mkdir -p "$HOME/Videos/Screencasts"
		#bf="$HOME/Videos/Screencasts/$(date +'%Y%m%dT%H%M%S')"
		mkdir -p "$OUTPUTDIR"
		bf="$OUTPUTDIR/$(date +'%Y%m%dT%H%M%S')"
		vidf="$bf.$VIDEOEXT"
		logf="$bf.log"
		if [ "$2" == "fullscreen" ]; then
			wf-recorder $WF_RECORDER_OPTS --file "$vidf" 1>"$logf" 2>&1 &
		elif [ "$2" == "region" ]; then
			sleep 1
			wf-recorder $WF_RECORDER_OPTS --geometry "$(slurp)" --file "$vidf" 1>"$logf" 2>&1 &
		elif [ "$2" == "display" ]; then
			sleep 1
			wf-recorder $WF_RECORDER_OPTS --geometry "$(slurp -r -o)" --file "$vidf" 1>"$logf" 2>&1 &
		else
			printf "Argument \$2='%s' not valid (fullscreen/region)" "$2"
			exit
		fi
		pid=$(jobs -p | tail -n 1)
		printf "%d\n%s\n%s" "$pid" "$vidf" "$logf" > "$pidf"
		disown "$pid"
	fi
	pkill -RTMIN+$SIGNAL waybar

else
	printf "ERROR: Argument %s not valid\n" "$1"
fi
